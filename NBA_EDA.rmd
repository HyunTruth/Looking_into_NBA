...
```{r echo=FALSE, D_PPM_FG}
ggplot(aes(x=FG., y=PPM), data = D) +
  geom_jitter(alpha = 0.2) +
  geom_smooth(method = "lm")+
  xlab("Field Goal Percentage")
```
...
This density plot add into the odd trend in field goal percentage and point per
...
range, with the median way above the other positions.

This implies that centers generally have higher field goal percentage when 
compared to other positions. It is reasonable, as centers, generally, shoot 
a lot closer to the rim than most of the other positions.
